            ...
            public override string ToString()
            {
                switch (_encoding)
                {
                    case MMSEncoding.MMSTO:
                        return $"mmsto:{_number}{(string.IsNullOrEmpty(_subject) ? string.Empty : $"?subject={Uri.EscapeDataString(_subject)}")}";
                    case MMSEncoding.MMS:
                        return $"mms:{_number}{(string.IsNullOrEmpty(_subject) ? string.Empty : $"?body={Uri.EscapeDataString(_subject)}")}";
                    default:
                        return string.Empty;
                }
            }
